...
export function TagPill({ tag, height, count, onRemove }: { tag: Tag; height: number; count?: number; onRemove?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {onRemove && (
        <Pressable onPress={onRemove} hitSlop={6} style={({ pressed }) => [{ alignSelf: "center" }, pressed && { opacity: 0.4 }]}>
          <Text style={{ color: tag.color, fontSize: fontSize * 1.1, fontWeight: "400", lineHeight: fontSize * 1.4, opacity: 0.55 }}>
            ×
          </Text>
        </Pressable>
      )}
