...
| `tag-pill.tsx` | A tag chip, colored from `tag.color`, led by its type icon (`TAG_TYPE_ICONS` from `@/lib/tag-values`), or a plain colored dot for basic tags. Used by the tags tab, tag detail, song rows, the song detail modal, and the simple query builder. |
| `create-tag-dialog.tsx` | Name + color picker, plus a type picker whose options show `TAG_TYPE_ICONS`. Calls `useCreateTag`. |
