import { QueryNode, QueryNodeLogic, PaletteItem, SlotAddress, LogicOperator } from "./types";
...
export function removeNode(root: QueryNode | null, targetId: string): QueryNode | null {
  if(!root) return null;
  if(root.id === targetId) return null;
  if(root.kind === "tag") return root;

  const nextChildren = root.children.map((c) => (c ? removeNode(c, targetId) : null));
...
export function findNodeById(root: QueryNode | null, id: string): QueryNode | null {
  if(!root) return null;
