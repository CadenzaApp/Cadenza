    ...
    AsyncFunction("authorize") { (developerToken: String) async throws -> [String: String] in
      guard #available(iOS 15.0, *) else {
        throw Exception(
          ...
          description: "Apple MusicKit requires iOS 15.0 or later."
        )
      }
      ...
      case .authorized:
        ...
        do {
          let userToken: String = try await withCheckedThrowingContinuation { continuation in
              ...
              if let error = error {
                ...
                continuation.resume(
                  throwing: NSError(
                    ...
                    code: -1,
                    userInfo: [NSLocalizedDescriptionKey: "requestUserToken returned neither a token nor an error."]
                  )
                )
              }
            }
